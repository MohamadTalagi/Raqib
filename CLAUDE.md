...
- **A real, non-cosmetic addition, not just a rename — and a real initial
  claim caught wrong by live verification, corrected rather than left
  standing**: `plc-gateway`'s Modbus server now answers function code 0x2B
  (Read Device Identification) via pymodbus's `ModbusDeviceIdentification`
  — a real Modicon PLC capability, unauthenticated by design, confirmed
  live via a direct pymodbus client call. The plan's own working assumption
  that this would also close a previously-documented gap (the 2026-08-02
  changelog entry's `modbus-discover` "open but no data" result) turned out
  to be **false once actually live-verified**: nmap's `modbus-discover.nse`
  only calls the 0x2B read as a follow-up after first getting a real
  response to function 0x11 (Report Slave ID), and pymodbus's server never
  answers 0x11 at all (confirmed live with a hand-crafted raw frame,
  cross-checked against a known-working 0x03 call on the identical
  framing/socket to rule out a framing bug) — root cause not pursued
  further, out of scope for a vendor-identity pass. The `modbus-discover`
  gap remains open, unchanged from before this session; full detail in
...
- **Verified**: 56 per-device fixture tests passing across all 6 devices
  ...
  pass and started as part of it. **Full live verification against the
  rebuilt stack, not just unit tests**: rebuilt all 6 device images +
  `telnet-sim`, brought the whole 16-container stack up, and directly
  probed every device's real identity surface — `/api/device/info` on all
  8, the Telnet banner (`device-insecure` + `telnet-sim`), a real raw SSDP
  M-SEARCH + `description.xml` fetch against `device-router-gw`, the real
  `broadcast-upnp-info` NSE script (byte-for-byte matched the fixture
  updated earlier the same session), a real mDNS TXT query against
  `device-speaker`, and a real RTSP DESCRIBE against `device-nvr` (correct
  new `Server:` header and SDP session name) — every one showed the new
  real vendor/model. Ran a real `POST /network-scans` sweep through the
  actual worker end to end (still classifies every host correctly,
  confirming the rename didn't touch classification logic) and two real
  `POST /scan-jobs` (`TEST-UPNP-PROBE` on `device-router-gw`,
  `TEST-MDNS-PROBE` on `device-speaker`), recording two real fresh evidence
  rows (`EV-2026-08-04-0001`/`-0002`) carrying the new Netgear/Sonos
  identity end to end. Found and fixed a real, live-only-visible gap this
  same pass: every device's DB-stored `display_name` and inventory
  `vendor`/`model` fields (separate from the device's own live-reported
  identity) still showed the old fictional names/were null — updated all 8
  via real `PATCH /devices/{id}` calls, then confirmed in an actual browser
  (Claude-in-Chrome) that the Devices list, a device detail page's
  Inventory card, and the new evidence row all render the correct new
  identity.
...
one genuinely unresolved finding — see below). Each task shipped as its own
independently-committed, independently-live-verified commit, per this
...
**What shipped, in order, each its own commit** (full task-by-task checklist,
exact function signatures, timeout-formula constants, and every
live-verification step are in the plan file itself, all now checked off with
their real outcome — nothing here should need re-deriving from scratch):
1. **Telnet/SSH confidence split** (`e0f58b0`) — a Telnet-open host (with or
   without SSH) now gets `confidence: "medium"`; an SSH-only host stays
   `"low"`. Widened the frontend `confidence` union to 3 values, added a
   small confidence indicator to `NetworkDiscoveryPanel` (previously never
   rendered at all). Live-verified against the real `telnet-sim` fixture.
2. **MAC address + maintained IEEE OUI-registry lookup** (`a1c3be4`) —
   `oui_lookup.py` (mirrors `cisa_kev.py`'s fetch/cache shape) resolves a
   host's MAC to a real vendor, falling back to nmap's own smaller bundled
   guess. **A real, live-caught finding**: the IEEE registry's own front-end
   418s on the default `requests` User-Agent (a WAF bot-block) — fixed with
   a browser-shaped header. Live-verified: the real 39,889-row registry
   fetched into the worker's cache, a real known OUI (Cisco) resolved
   correctly, and every one of this lab's own container MACs correctly
   resolved to `null` (Docker's virtual MACs genuinely have no registry
   entry). Also caught and fixed a real, unrelated pre-existing test gap
   during this same live pass: `NetworkDiscoveryPanel.test.tsx` never wrapped
   the panel in a Router, masked only because its unmocked fetch always
   failed with no live API on the test host — it started crashing the
   instant this session's own live-verification stack came up on
   `localhost:8000`.
3. **SSDP/mDNS broadcast fold-in** (`4702d78`) — `broadcast-upnp-info`/
   `broadcast-dns-service-discovery` folded into the sweep so a UDP-only
   device (no TCP signature port at all) is no longer invisible.
   **Live-first verification surfaced two real, unplanned fixture bugs**,
   not guessed from nmap's docs: neither `device-router-gw`'s SSDP responder
   nor `device-speaker`'s mDNS responder ever joined its multicast group
   (`IP_ADD_MEMBERSHIP`) — real broadcast queries got zero response even
   though this lab's own unicast probes already worked; and once UPnP
   discovery could reach `device-router-gw` at all, its SSDP response's
   `LOCATION` header echoed the *requester's* address instead of its own,
   and no `/description.xml` endpoint existed — so the real UPnP discovery
   flow was broken end to end for any real client, not just nmap. Both
   fixed. Live-verified end to end: `device-router-gw` now completes the
   whole real UPnP flow (M-SEARCH → LOCATION → fetch description.xml).
   Honestly documented what could *not* be live-verified: mDNS/DNS-SD
   enumeration, since `device-speaker`'s intentionally minimal mDNS
   responder doesn't implement the DNS-SD PTR-enumeration convention that
   script needs — it correctly returns nothing, confirmed live, not a bug.
4. **Two-phase discovery for scale** (`2fc8283`) — Stage A (a fast
   whole-scope `-sn -n` ping/ARP sweep) finds live addresses; Stage B (the
   full `-sV` scan, broadcast scripts included) targets only those
   addresses, never the whole scope — what actually makes the platform's own
   /16 scope ceiling practical. New shared `job_runner.py` orchestrator
   (`_run_network_discovery`) replaces the old single-command dispatch for
   this one test_id. **The owner's explicit choice** over a simpler "just
   raise the timeout ceiling" fix. **A live large-scope verification attempt
   surfaced a real, only partially understood anomaly, documented honestly
   rather than glossed over** (`docs/errors/033`): an unrouted RFC1918 range
   used as a "large empty scope" proxy caused a real Stage A job timeout
   against this task's first-draft estimate, and a smaller isolated check of
   the same range showed every address falsely reporting "up" — likely a
   Docker Desktop host-networking artifact for a destination with no real
   route, not conclusively root-caused within this task. Responded by
   substantially raising the timeout constants from this real (if
   incomplete) worst-case data, then re-verified the real always-supported
   case (the audit-network `/24` alone) still reproduces byte-for-byte-
   equivalent results to the pre-Task-4 single-stage sweep. What remains
   genuinely unverified (Stage B at true large-live-host-count scale, a real
   full `/16` sweep, and whether Stage A can be trusted not to over-report on
   an unusually-routed network) is stated plainly in
   `docs/known-limitations.md`, not implied as settled.

**Full regression after all 4 tasks**: 414 `policies` (+27 net this session)
+ 333 `lab/auditor/worker`-in-container (bind-mounted, no rebuild needed) +
334 `lab/auditor/api` (3 pre-existing WeasyPrint failures, unrelated) + 315
frontend tests, `tsc -b`/`oxlint` clean throughout. `auditor-worker`
restarted after each task (bind-mounted code); every task live-verified
against the real running stack, not just unit tests, including two full
end-to-end `POST /network-scans` runs proving the final two-stage
architecture reproduces the pre-Task-4 sweep's results exactly for this
lab's real fleet.
...
| Date | Change |
|---|---|
| 2026-08-11 | **NCA suggestions can now suggest `pass`, closing a gap where clean evidence produced nothing at all.** Every finding mapping fired on an *insecure* condition, so a device that genuinely passed a check was indistinguishable from one never tested - both sat at `not_tested` with nothing actionable, and only failures ever reached an auditor. 18 pass-condition mappings (`verdict_hint = 'pass'`, allowed by new migration **017**) are each the exact inverse of a fail rule on the same observation field, so `_field_present` still guarantees they only fire on evidence that genuinely carries that observation - a positive signal, never inferred from silence. **Precedence is fail > review_required > pass**, confirmed live on `device-partial`, whose 2-4-3 matches `weak_cipher` (fail) and `tls_version` (pass) and correctly resolves to **fail**. **One clean aspect is enough to suggest pass** - symmetric with fail, the owner's explicit choice over a stricter "every applicable aspect must have been checked" rule, put to them via AskUserQuestion along with the auto-record question rather than guessed. Two things bound its real trade-off (2-4-3 has six independent aspects, so one clean TLS reading can suggest pass while five were never run): every suggestion now carries **`checked_aspects`**, surfaced in `RecordAssessmentDialog` on a suggested pass ("Based on 1 check that actually ran: `tls_version` … confirm that is enough before signing"), so the person certifying sees the basis rather than a bare word; and **a pass mapping is only seeded where a clean reading proves the probe ran**. Three obvious inverses were deliberately dropped after reading the parsers, because each reads clean when the check never happened: `weak_cipher == False` is a substring test so a refused connection looks identical to a strong cert; `tls_version not_in [deprecated]` is True for `None`; and `plaintext_get_visible == False` hits this project's own documented zero-packet capture flake (`docs/errors/023`). A `match_rule` is a single predicate (no conjunction in `policy_engine.py`) so "clean AND the probe ran" is inexpressible, and **2-4-3 is a blocking guideline** where a false pass flips a device's whole readiness - so the positive rule `tls_version in ['TLSv1.2','TLSv1.3']` is used instead, with a regression test pinning the failed-handshake case. **The Fully Automated Run never auto-records a suggested pass** (fail/review_required still record; skipped passes surface as `nca_passes_left_for_review`) - a machine may flag a suspected problem unattended, but only a person certifies compliance. The handoff claimed no runner change was needed either way; that was wrong - `_run_nca_compliance` recorded `suggested_status` verbatim, so passes would have been auto-recorded. **Nine existing tests encoded "clean evidence maps to nothing" as a deliberate fact** and failed as designed; each was rewritten to assert the invariant it actually protects (a clean reading must never become a *failing* suggestion) rather than an emptiness that is no longer true. Verified live: `device-hardened` now yields 4 real passes (Telnet closed, default creds rejected, current TLS negotiated, monitoring endpoint present) where it previously yielded none; `device-insecure`'s 8 fails are unchanged; a scoped Fully Automated Run recorded 7 and left 4 passes for review with 0 errors, and `SELECT count(*) ... WHERE auto_recorded AND status='pass'` returns **0** fleet-wide. 496 `policies` + 371 `lab/auditor/api` + 213 `lab/auditor/worker` + 336 frontend tests (52 files) passing, `tsc -b`/`oxlint` clean. |
| 2026-08-11 | **SA-IOT Compliance retired as a pipeline stage; its one unique capability moved into NCA Compliance; risk scoring made NCA-only.** The owner's call: SA-IOT was redundant with the fuller 81-guideline NCA framework, with 4 of its 5 controls already covered there. Scope was confirmed up front as **stage + UI only** - `policy_engine.py`, the five `SA-IOT-*.yaml` controls, the `verdicts` table and the `/verdicts`//`/controls` pages all stay, because that engine is the mentor's Day-3 graded deliverable and the reports still read it. **(1) The missing capability, device identification, is now real NCA automation**: CGIoT **2-1-1** ("maintain an inventory of IoT devices... including naming, classification") was `organization`/`manual` with no evidence hooks; it is reclassified `device`/`hybrid` with a new finding mapping from `TEST-DEVICE-ID`'s `device_identified` observation - the same Phase-0 precedent used for 2-6-2/2-6-3/2-7-2. Verified live: 2-1-1 now returns a real auto-suggestion backed by 3 real evidence records. Its `verdict_hint` is **`review_required`, not `pass`** - partly because the DB CHECK constraint permits only fail/review_required, but mainly because it is the honest hint: the guideline also requires the *organization* to maintain the inventory, and the collector only proves a device *discloses* an identity, not that the values are accurate. **NCA coverage is unchanged at 76/81 but better distributed - 17 automated/hybrid + 59 checklist, up from 16 + 60** - device identification moved out of the manual column. **(2) The 18 `sa_iot_compliance`-tagged collectors were rehomed, not deleted**: they were always the evidence behind NCA's own finding mappings, so the phase constant was renamed to `nca_compliance` and `NCACompliancePage` gained a "Collect compliance evidence" runner (DeviceCohortPicker + PhaseRunnerCard). Without that they would have been reachable only through a Fully Automated Run - a real capability regression the deletion would otherwise have caused. `/sa-iot-compliance` now 302s to `/nca-compliance` rather than 404ing. **(3) Risk scoring is NCA-only**: `compliance_score` already used NCA's `device_score()`, so the only SA-IOT contamination was `violation_count = NCA fails + SA-IOT FAIL verdicts`. With SA-IOT gone that factor would have counted only NCA failures - the same failures the NCA compliance score already reflects - so it **double-counted one framework and was removed**, its 0.05 folded into `compliance` (**0.25 -> 0.30**), which is also the requested increase in NCA weight. Risk is now 6 factors, not 7. **Three real regressions caught by the suites rather than by inspection**: the Jinja PDF/HTML report template still rendered `breakdown.violations` (4 report tests failed with `UndefinedError`); `test_seed_checklists.py` caught 2-1-1 keeping a now-unreachable checklist after its reclassification - exactly what that test was written to catch - and a stale checklist row left in the live DB was double-counting coverage as 77/81 until deleted; and `App.test.tsx`/`pipeline.test.ts` pinned the old 6-phase order. 495 `policies` + 212 `lab/auditor/worker` + 367 `lab/auditor/api` tests passing, `tsc -b`/`oxlint` clean. |
| 2026-08-10 | **Firmware Currency / EOL Check — answers whether a device's firmware is actually behind a published fix**, implemented from the new `handoff.txt` plan. See `docs/vulnerability-intelligence.md`'s new "Firmware currency" section. **The source was data already being fetched and discarded**: NVD's CVE records carry per-CPE version-range fields that `nvd_lookup._summarize_cve` was trimming out, so this needs no new external source, no new network call and no per-vendor scraper. New pure `policies/catalog/firmware_version_compare.py` (hand-rolled, no new dependency - `packaging` is importable in the worker only as a transitive dep and pinned in neither requirements file, confirmed live). Secondary source: `schneider_csaf.py` fetches Schneider Electric's real CSAF advisories from CISA's `cisagov/CSAF` mirror on the same out-of-band cadence as every other feed, consulted *before* NVD for that one device since a vendor's own advisory outranks NVD's transcription; `advisory_source` records which one answered. **The plan was audited against live data before any code was written (`handoff.txt` §9) and five things did not hold.** The most consequential: §6.1's conservative rule (an unbounded CVE never counts toward `outdated`) would have reported **four of seven** products as `unknown` - including Hikvision's KEV-listed, actively-exploited CVE-2021-36260 - because those products' CVEs carry no version bounds at all. §7 item 4 asked for exactly this to be revisited; the owner chose a **fourth status, `affected_no_fix`**, which keeps the discipline (an unbounded CVE still isn't "you're behind a fix") while refusing to bury a real finding behind "we couldn't tell". Also caught: §6.1's `parse_version` regex returned `None` for Axis's real `"AXIS OS 11.11.100"` (the prefix pattern required letters *immediately* against a digit, and §5.2 never recorded that string); Axis's hardware-only `h:` CPE marks all 6 of its CVEs `vulnerable: false`, so the check is **structurally** a no-op there regardless of parsing; NVD boundary values themselves carry suffixes (`versionEndIncluding: "1.0.7.2_1.1.93"`), so the discard-the-suffix simplification applies to both sides; and §7 item 2's suspicion that the CSAF advisory table "under-covers" is wrong - CVE-2024-11737's only NVD reference is a Schneider **PDF**, so CSAF genuinely covers one advisory, one CVE, one device. Verified live end to end against all 8 fixtures with real NVD data: **device-router-gw reports `outdated` with 12 affected CVEs and the real earliest fix `1.0.11.134`** (device is on `1.0.11.132`), Schneider/Hikvision×2/Yale/Sonos report `affected_no_fix`, Axis reports `unknown`, Dahua reports no CPE mapping - all four statuses exercised by real data. Cache grew to 139.7 KB (§7 item 6, no meaningful inflation). 498 `policies` (+60) + 212 `lab/auditor/worker` (+14) + 368 `lab/auditor/api` (+3) + 341 frontend tests (+3) passing, `tsc -b`/`oxlint` clean. |
| 2026-08-06 | **Navigation cleanup ahead of a demo: Scan Console removed, Device Console moved, and a default-credentials button dropped.** `ScanConsolePage` is gone entirely - page, route, sidebar entry and tests (435 lines). With both its items removed the **"Advanced Tools" group had nothing left**, and the label overstated what a single device inspector is, so the group went too. **Device Console moved to Records** rather than Pipeline (the owner's choice between three offered placements): Pipeline is deliberately ordered top-to-bottom as the 10-stage sequence and Device Console is not a stage, while Records already holds Network Map - likewise a live view rather than a stored record - so that group reads as "inspect things" in practice. Separately, the **"Login (admin / admin)" button was removed from Device Console**: it was the only POST among its eight endpoints and the one control that sent a credential pair from the browser, duplicating far more crudely what `TEST-AUTH-DEFAULT-CREDS` already does properly (10 documented pairs, through the worker, recorded as real evidence behind SA-IOT-002). "Login page" - a GET that opens the real rendered page - stays. **Two stale user-facing strings found while checking for lingering references, both naming pages that no longer exist**: `main.py`'s assess-verdict 400 told users to run a test from "Run Scan or the Scan Console" (Run Scan was deleted in the 2026-08-01 dashboard overhaul, so it had already been naming one dead page), and `DeviceDetailPage`'s Assessment history caption said the same - both now name the pipeline phase page that actually runs those tests. The second was caught only by checking the **deployed bundle** rather than trusting a source grep, which had used a non-recursing path glob and wrongly reported "none". Verified against the rebuilt image that the served bundle contains no "Scan Console", "Advanced Tools", "Login (admin / admin)" or `username=admin&password=admin`. 338 frontend tests across 53 files (down exactly the 6 tests and 1 file of the deleted page), 11 assess-verdict API tests, `tsc -b`/`oxlint` clean; `auditor-web`/`auditor-api` rebuilt and redeployed. |
| 2026-08-06 | **New `TEST-DEVICE-MAC-VENDOR` scan — auto-extracts a device's MAC and identifies the organization its OUI is registered to** (owner-requested, via the macvendors.com API). See `docs/vulnerability-intelligence.md`'s new "MAC vendor lookup" section. A dedicated Fingerprinting-phase scan with no manual input: the collector reads the MAC from the device's own `GET /api/device/info`, then resolves it through three tiers - local per-MAC cache → the cached IEEE MA-L registry (already maintained by `job_runner`) → a live `api.macvendors.com` call, cached after. **The comparison is the point, not the lookup**: it reports both the vendor a device *claims* and the vendor its MAC prefix is *registered to*, plus whether they agree, since a device asserting one while carrying another's OUI is a real signal that neither value could show alone. New `vendor_names_agree()` does that properly (strip corporate suffixes/punctuation, then whole-word containment) because these names never match literally - `Hikvision` vs `Hangzhou Hikvision Digital Technology Co.,Ltd.`, `Yale` vs `Assa Abloy AB - Yale`. Its three-valued return matters: None ("could not compare") must never render as False ("these disagree"). **Measured live before designing, not assumed**: no API key needed, all 8 fixture MACs resolve, an unknown OUI returns a clean 404 (verified with a real Docker MAC from this project's own committed evidence) which is cached as a definite "not registered", and the free tier is strictly ~1 req/s - a second immediate request 429s, hence the pacing and single retry. A failed request is deliberately never cached, so a transient outage can't poison the cache into a permanent wrong answer. Honest framings kept throughout: an unregistered OUI is stated as expected-not-suspicious (every Docker MAC is locally-administered), and a mismatch is written as a prompt offering the innocent readings (ODM manufacturing, a reused NIC) rather than a spoofing verdict. Verified live end to end: all 8 fixtures scanned through the real API/worker with every OUI correctly corroborating its device's claim; the live macvendors tier separately exercised inside the worker with the two offline tiers disabled (4 real vendors resolved, the Docker MAC returning checked-and-unregistered, and a second pass served entirely from cache with zero network calls); and the whole flow driven through the real dashboard in a browser. **A known gap, stated rather than left implied**: no fixture is deliberately mislabelled, so the MISMATCH branch is covered by unit tests built from real name pairs, not by a live run. 449 `policies` (+11) + 198 `lab/auditor/worker` in-container (+21) + 362 `lab/auditor/api` tests passing. |
