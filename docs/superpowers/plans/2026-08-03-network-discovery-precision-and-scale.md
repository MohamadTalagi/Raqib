...
- [ ] Add a MAC-address regex to `_parse_network_discovery_observations()` (alongside the existing per-port regex, same per-host block scope): match `^MAC Address: ([0-9A-Fa-f:]+) \((.+)\)$`, capturing nmap's own address + vendor guess per host block.
- [ ] Build `lab/auditor/worker/scan_scripts/oui_lookup.py`, mirroring `cisa_kev.py` exactly:
  - Fetches IEEE's public MA-L registry (`https://standards-oui.ieee.org/oui/oui.csv`, no auth needed — confirm reachability from the worker container during implementation, since `cisa_kev.py` already proves this container has real internet egress).
  - Caches it locally (new named volume or reuse `document-store`, decide during implementation which is simpler given the existing `grype-db-data`/KEV-cache precedent).
  - A staleness check on the same low-frequency cadence pattern as `maybe_refresh_grype_db` in `job_runner.py`.
  - `lookup_vendor(mac_address: str) -> str | None` — looks up the OUI (first 3 octets) against the cached registry; returns `None` (never a guess) if the cache is empty/stale-and-unreachable or the OUI genuinely isn't in the registry (e.g. a locally-administered/random MAC, which the registry correctly has no entry for — this is the honest, expected Docker-lab result).
- [ ] In `job_runner.py`, wire `oui_lookup.py`'s refresh into the existing periodic-refresh poll pattern (same place `maybe_refresh_grype_db`/`maybe_refresh_network_scope` already live).
- [ ] Fold into observations: each host gets `mac_address`, and `mac_vendor` resolved by trying the IEEE registry first, falling back to nmap's own bundled guess if the registry lookup returns `None` (never silently drop nmap's own answer), plus `mac_vendor_source` recording which one actually answered — this is the honest provenance trail, matching every other tool-attribution field in this codebase (`collector_versions`, `tool_version`).
- [ ] `types.ts`: add `mac_address`, `mac_vendor`, `mac_vendor_source` (optional) to `DiscoveredHost`.
- [ ] `NetworkDiscoveryPanel.tsx`: render the MAC + vendor when present, with a short inline note when vendor is `null`/"Unknown" explaining a locally-administered MAC can't be vendor-resolved (correct in this Docker lab, honest on a real VLAN too).
- [ ] Tests: `test_oui_lookup.py` (mocked HTTP fetch, cache staleness, fallback path, unknown-OUI → `None`), plus `test_scan_tests.py` cases confirming the parser extracts MAC+vendor from a real-shaped block (use the actual `EV-2026-07-23-0001.txt` MAC lines as fixture text, not invented ones).
- [ ] Update `docs/known-limitations.md`: replace "Deliberately does not use MAC-vendor (OUI) lookup" with an accurate description of what's now captured, keeping the honest caveat that Docker's virtual MACs mean this lab will (correctly) show "Unknown"/`null` for its own real device fleet — the value is for a real physical VLAN deployment, demonstrated by this fix, not provable end-to-end inside this lab.
- [ ] Live-verify: run a real sweep, confirm the same MAC addresses `EV-2026-07-23-0001.txt` already recorded reappear in the new `mac_address` field, and `mac_vendor` honestly resolves to `null`/"Unknown" via both the IEEE-registry path (genuinely has no entry for a locally-administered OUI, first bit of the first octet set) and the nmap-bundled fallback path (confirm both code paths actually execute, e.g. by temporarily breaking one to confirm fallback fires, then restoring it).
