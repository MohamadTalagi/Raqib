...
| [031](031-firmware-upload-only-supported-tar-gz-not-zip.md) | Firmware upload only handled `.tar.gz` (validation, storage, YARA scan, and all 7 TEST-FW-* checks hardcoded `tarfile`); a `.zip` — what Windows produces by default — had no path through the pipeline | auditor-api + auditor-worker | 2026-07-27 | resolved |
| [032](032-docker-desktop-embedded-dns-corruption-survived-reboot.md) | Docker Desktop's embedded DNS resolver returned inconsistent per-name results (some names NXDOMAIN, others total timeout) on one bridge network only, surviving container/network recreation, WSL restart, Docker Desktop restart, and a full OS reboot — fixed only by a full Docker Desktop data purge | docker-compose / Docker Desktop | 2026-08-01 | resolved |
| [033](033-unrouted-scope-nmap-timing-anomaly.md) | An unrouted large-scope proxy (used to live-verify Task 4's two-phase discovery timeout formula) caused a real Stage A timeout, then a smaller isolated check of the same range showed every address falsely reporting "up" — root cause not conclusively determined, mitigated with a much more conservative timeout constant | policies/catalog/scan_tests.py / lab/auditor/worker/job_runner.py | 2026-08-03 | open |
