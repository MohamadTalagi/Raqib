...
describe("applicableDomains", () => {
  it("excludes a domain with zero controls across every status", () => {
    const domains: NCADomainSummary = {
      ...
      "Cybersecurity Defense": { pass: 3, partial: 0, fail: 1, not_tested: 2, review_required: 0 },
    };
  ...
  it("keeps a domain whose only nonzero count is not_tested", () => {
    ...
    const domains: NCADomainSummary = {
      "Cybersecurity Resilience": { pass: 0, partial: 0, fail: 0, not_tested: 5, review_required: 0 },
    };
    expect(applicableDomains(domains)).toHaveLength(1);
  });

  it("keeps a domain whose only nonzero count is review_required", () => {
    const domains: NCADomainSummary = {
      "Cybersecurity Resilience": { pass: 0, partial: 0, fail: 0, not_tested: 0, review_required: 2 },
    };
    expect(applicableDomains(domains)).toHaveLength(1);
  });
